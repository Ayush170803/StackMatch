...
requestRouter.post('/request/send/:status/:toUserId',userauth, async (req,res) =>
{
    ...
    const existingreq = await connection.findOne({
        $or:[{fromUserId:fromUserId,toUserId:toUserId}, //no repeat
            {fromUserId:toUserId,toUserId:fromUserId}, //no existing
        ]
    })
...
{
    return res.status(400).json({
        message: "User Doesn't Exists wrong id"
    });
}
...
requestRouter.post('/request/review/:status/:requestId',userauth, async(req,res)=>
{
    try
    {
        ...
        const isconnectionrequest = await connection.findOne({
            _id:requestId,
